# -*- coding: utf-8 -*-
"""Email Spam Classification-DecisionTree.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MRuvJId3Mp_ynszQxx1r2nhWmIae_RpI
...
###Objective
###1) Data Preprocessing: Handle missing values, encode text features, and prepare the data for modeling.
###2)Exploratory Data Analysis (EDA): Examine the distribution of features and their relationships to the target variable.
###3) Model Building: Implement a decision tree classifier to predict spam versus non-spam emails.
###4) Model Evaluation: Assess the classifier’s performance using metrics like accuracy, confusion matrix, and classification report.
...
df= df.iloc[:,1:] # Select all rows and all columns except the first one from the DataFrame

df

x = df.iloc[:, :3000].values  # Extract the first 3000 columns as a NumPy array for feature selection
x #The x variable now holds the feature data for the first 3000 columns of the DataFrame.

y = df.iloc[:, -1].values  # Extract the last column as a NumPy array for the target variable
y #The y variable now contains the target variable data from the last column of the DataFrame.

"""##Splitting the data for Train and Test"""

x_train, x_test, y_train, y_test = train_test_split(x, y, test_size = 0.2, random_state= 1) #Split data into training and testing sets with 20% of data for testing
